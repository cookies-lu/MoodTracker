...
angry.innerHTML = EMOTIONS.Angry.icon;
sad.innerHTML = EMOTIONS.Sad.icon;
neutral.innerHTML = EMOTIONS.Neutral.icon;
happy.innerHTML = EMOTIONS.Happy.icon;
excited.innerHTML = EMOTIONS.Excited.icon;
...
//saveBtn to save Mood to arweave
...
filechoose.onchange = function(e) {
  ...
  var moodVisible = document.getElementById("Mood");
  var saveVisible = document.getElementById("saveBtn");
      ...
      try {
        ...
        arweave.wallets.jwkToAddress(wallet).then((address) => {
          loginHandler(wallet, address);
            ...
            moodVisible.classList.remove("hidden");
			moodVisible.classList.remove("displaynone");
			saveVisible.classList.remove("hidden");
			saveVisible.classList.remove("displaynone");
  ...
  async function getMood() {
    ...
    // Decode saved data(CSV string) to JSONObject
    var  bufferString = text;
    ...
    for (const mood in savedMood) {
        ...
        savedMoods[0].objs[0].icon = savedMood[mood].icon;
        savedMoods[0].objs[0].date = savedMood[mood].date;
        moodis.push([savedMoods[0].objs[0].icon, savedMoods[0].objs[0].date])
	...
	for (const x in savedMood) {
		  graphIcons.push(savedMood[x].icon);
		  graphDates.push(savedMood[x].date);
	  };
  ...
  async function saveMood() {
    if (state.loggedIn) {
	  ...
	  var saveMood = convertArrayToCSV(moodis,{ header, seperator:','});	
